...
typesP.getSupertypeNames = function(typeName) {
    assert.ok(hasOwn.call(types.defCache, typeName));
...
// object.
typesP.computeSupertypeLookupTable = function(candidates) {
    var table = {};

    for (var typeName in types.defCache) {
        if (hasOwn.call(types.defCache, typeName)) {
            var d = types.defCache[typeName];
            assert.strictEqual(d.finalized, true);
...
typesP.defineMethod = function(name, func) {
    var old = types.nodePrototype[name];

    // Pass undefined as func to delete nodePrototype[name].
    if (isUndefined.check(func)) {
        delete types.nodePrototype[name];

    } else {
        isFunction.assert(func);

        Object.defineProperty(types.nodePrototype, name, {
...
typesP.getFieldNames = getFieldNames;
...
typesP.getFieldValue = getFieldValue;

// Iterate over all defined fields of an object, including those missing
// or undefined, passing each field name and effective value (as returned
// by getFieldValue) to the callback. If the object has no corresponding
// Def, the callback will never be called.
typesP.eachField = function(object, callback, context) {
...
typesP.someField = function(object, callback, context) {
...
types.finalize = function() {
    Object.keys(types.defCache).forEach(function(name) {
        types.defCache[name].finalize();
    });
};
