...
Dp.finalize = function() {
    ...
    if (!self.finalized) {
        ...
        if (self.buildable) {
            ...
            var paramCount = self.buildParams.length;

            Object.defineProperty(buildFn, "paramCount", {value: paramCount});

            var required = false;

            for (var i = paramCount - 1; i >= 0; i--) {
                var param = self.buildParams[i];
                var field = self.allFields[param];
                required = required || !field.defaultFn;
                var paramDef = {};

                var props = {
                    name : {enumerable: true, value: param},
                    type: {enumerable: true, value: field.type},
                    required: {enumerable: true, value: required}
                };

                Object.defineProperties(paramDef, props);
