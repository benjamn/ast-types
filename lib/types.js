...
Type.or = function(/* type1, type2, ... */) {
    ...
    return new Type(function(value, deep) {
        for (var i = 0; i < len; ++i)
            // In case the deep parameter is a function (for debugging
            // type errors), we don't want to complain just because one
            // alternative did not pass, so coerce deep to a boolean.
    ...
    Object.defineProperties(self, {
        ...
        type: {
            value: new Type(function(value, deep) {
                var result = self.check(value, deep);
                if (!result && isFunction.check(deep))
                    deep(self.type, value);
                return result;
            }, typeName)
        }
    });
...
Dp.checkAllFields = function(value, deep) {
    ...
    function checkFieldByName(name) {
        ...
        var child = field.defaultFn.call(value); // TODO Can't expose
                                                // builder arguments.
...
Dp.check = function(value, deep) {
    ...
    return !!vDef // Coerce to boolean.
        ...
        && (!deep || (vDef.checkAllFields(value, deep) &&
